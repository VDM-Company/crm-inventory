...
import type { ReceiptListItem, ReceiptStatus } from '~/types'
...
const statusMap: Record<ReceiptStatus, { label: string, color: 'neutral' | 'warning' | 'info' | 'primary' }> = {
...
const columns: TableColumn<ReceiptListItem>[] = [{
  ...
  cell: ({ row }) => h('div', { class: 'flex items-center gap-1' }, [
    h(UButton, {
      ...
      variant: 'outline',
      size: 'xs',
      to: `/operations/receipts/${row.original.id}`
    }),
    h(UButton, {
